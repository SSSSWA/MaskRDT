...
from gyms.maskrdt.models.sequences import TokenSeq
...
class RandomLengthMask(SpanPred):
    """
    ...
    def get_input_masks_state_action(num_seq_1, seq_lens):
        ...
        possible_span_limits = []
        start = 0
        for end in np.arange(start, int((seq_lens + 1)/2)):
            possible_span_limits.append((start, end))

        span_indices = np.random.choice(len(possible_span_limits), size=num_seq_1)
        span_limits_n = [possible_span_limits[idx] for idx in span_indices]

        mask_size = (num_seq_1, seq_lens)
        s_mask = torch.ones(mask_size)
        a_mask = torch.ones(mask_size)

        for traj_idx, span_limits in enumerate(span_limits_n):
            # Mask out the things we want to predict (i.e. zero out)
            a, b = span_limits
            # mask from the start of the sequence to have different sequence length
            s_mask[traj_idx, a:b] = 0
            a_mask[traj_idx, a:b] = 0
            a_mask[traj_idx, seq_lens-1:seq_lens] = 0
